...
/// <summary>
/// 问题跟踪仓储。连接生命周期样板由 <see cref="SqliteRepository.ExecuteAsync"/> 统一管理。
/// </summary>
public class IssueRepository : SqliteRepository
{
    public Task<IEnumerable<Issue>> GetAllAsync()
        => ExecuteAsync(c => c.QueryAsync<Issue>(
    ...
    public Task<int> GetTotalCountAsync()
        => ExecuteAsync(c => c.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM Issue WHERE IsDeleted = 0"));
    ...
    public Task<IEnumerable<Issue>> SearchAsync(string keyword)
        => ExecuteAsync(c => c.QueryAsync<Issue>(
    ...
    public Task<IEnumerable<Issue>> GetByProjectAsync(string project)
        => ExecuteAsync(c => c.QueryAsync<Issue>(
            "SELECT * FROM Issue WHERE IsDeleted = 0 AND ProjectName = @Project ORDER BY CreateTime DESC",
            new { Project = project }));

    public Task<int> InsertAsync(Issue issue)
    ...
    public Task<int> UpdateAsync(Issue issue)
        => ExecuteAsync(c => c.ExecuteAsync(@"
    ...
    /// </summary>
    public Task<(IEnumerable<Issue> Items, int Total)> GetFilteredPagedAsync(
        ...
        => ExecuteAsync(async c =>
        {
            ...
            var total = await c.ExecuteScalarAsync<int>($"SELECT COUNT(*) FROM Issue {whereSql}", param);

            var dataSql = $"SELECT * FROM Issue {whereSql} ORDER BY CreateTime DESC LIMIT @Limit OFFSET @Offset";
            param.Add("Limit", limit);
            param.Add("Offset", offset);
            var items = await c.QueryAsync<Issue>(dataSql, param);
            return (items, total);
        });

    /// <summary>取回收站中的已删除问题（IsDeleted = 1），按删除时间倒序</summary>
    public Task<IEnumerable<Issue>> GetDeletedAsync()
        => ExecuteAsync(c => c.QueryAsync<Issue>(
